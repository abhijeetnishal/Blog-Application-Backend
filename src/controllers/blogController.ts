...
const getAllBlogPosts = async (req: Request, res: Response) => {
    try {
        ...
        if (sort && category) {
            ...
            const { rows } = await db.client.query(queury);
...
const getSpecificBlogPost = async (req: Request, res: Response) => {
    try {
        ...
        //get details of blog post
        const { rows } = await db.client.query(
            `SELECT * FROM blog_post WHERE _id = $1`, [blogPostId]
        );
    ...
    try {
        ...
        if (!author || !category || !content) {
            ...
            await db.client.query(
                `INSERT INTO blog_post(author, name, category, content, image_url) 
...
const updateSpecificBlogPost = async (req: Request, res: Response) => {
    try {
        //get post Id from params
        const blogPostId = req.params.id;
        ...
        if (rows.length === 0) {
            ...
            await db.client.query(
                ...
                WHERE _id = $6`,
                [author || currentValues.author, name || currentValues.name, category || currentValues.category, content || currentValues.content, image_url || currentValues.image_url, blogPostId]
            )
...
const deleteSpecificBlogPost = async (req: Request, res: Response) => {
    //get post Id from params
    const blogPostId = req.params.id;
    ...
    try {
        ...
        if (rows.length === 0) {
            ...
            //delete post
            await db.client.query(
                `DELETE FROM blog_post WHERE _id = $1`, [blogPostId]
            );
